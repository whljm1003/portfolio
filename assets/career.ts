...
export const stevelabsData: StevelabsProject[] = [
  {
    ...
    works: [
      ...
      "네이티브 앱과 웹 간 데이터 연동을 위한 브릿지 로직 설계 및 적용 (카메라/파일 기능, 로그인 유지, 소셜 로그인)",
      "모두싸인 API 연동을 통한 전자 계약 자동화로 계약 체결 시간 단축 및 업무 효율성 증대",
      "렌더링 최적화 및 UI 개선을 통해 페이지 로딩 속도 30% 향상",
      "Redux Toolkit 및 persist 적용을 통해 새로고침 및 뒤로 가기 시 데이터 유지 문제 해결",
      "React-Query를 활용한 데이터 페칭 최적화로 페이지 전환 성능 개선",
  ...
  },
  {
    ...
    works: [
      "Vanilla JavaScript를 활용한 DOM 조작으로 장바구니 및 결제 페이지의 UI를 동적으로 개선",
      "장바구니 내 배송비 및 할인 로직을 모듈화하여 정확성을 높이고 코드 유지보수성 향상",
      "코드 디버깅과 로직 단위별 모듈화를 통해 결제 시 데이터 불일치 문제를 효과적으로 해결하여 결제 안정성 확보",
    ],
    links: {
      notion:
  ...
  },
  {
    ...
    works: [
      "react-pdf/renderer 활용하여 검진 동의서, 의뢰서, 유전자 검사 결과 보고서를 PDF로 생성 및 다운로드 기능 구현",
      "모바일 환경 대응을 위한 react-pdf viewer 적용으로 iOS PDF 뷰어 문제 해결",
  ...
  },
  {
    ...
    works: [
      "React Spring Bottom Sheet를 사용하여 iOS 스타일의 메인 페이지 바텀시트 UI를 구현하여 모바일 사용성을 향상",
      "이미지가 많은 페이지의 렌더링 속도 저하 문제를 해결하기 위해 무한스크롤(Infinite Scroll) 및 Lazy Loading을 적용하여 이미지 로딩을 최적화",
      "예약 및 회원 관리 시스템을 구축하여 로그인, 예약 관리, 마이페이지 기능을 구현하고 사용자 편의성 증대",
      "관리자 페이지에서 일반 회원, 패프, 리뷰 관리를 위한 기능 개발로 서비스 운영 효율성 향상",
      "Toss Payments 결제 및 포트원 본인 인증 연동을 통해 예약 기능의 신뢰성 및 안정성을 확보",
      "카카오 소셜 로그인 기능 구현으로 사용자 접근성 및 가입 절차 간소화",
    ],
    links: {
      notion:
  ...
  },
  {
    ...
    works: [
      "재기획 및 디자인 조정을 통해 부족한 기획 보완 및 개발자 주도로 UI 개발",
      "요구사항 정리 및 일정 조율, 우선순위 관리로 프로젝트 진행 최적화",
      "가맹점 및 본사 관리자 기능 개발 (캘린더 기능, 결제 시스템(NICE페이먼츠) 적용)",
      "가맹점 및 본사 관리자 쇼핑몰 기능 구현을 통해 가맹점에서 상품 구매 가능하도록 개선",
      "선생님 & 학부모용 웹앱 개발 (퍼블리싱, API 연동, 웹앱 패키징) 및 네이티브 앱과의 브릿지 작업",
      "3000만 건 가량의 데이터 이관 작업 지원 및 발생한 오류를 수정하여, 올바르게 변환된 데이터를 유지",
    ],
    links: {
      notion:
  ...
  },
  {
    ...
    works: [
      "금 현물 거래 시 부가세와 소매 마진을 고려한 가격 정책을 수립하고, 종로 금 시장 현장 조사를 통해 실거래 환경을 분석하여 거래 프로세스를 개선",
      "금 도매·소매업체와의 현장 협업을 통해 거래 프로세스를 최적화하고, 실제 20~30돈 이상의 판매를 성공적으로 수행하여 플랫폼의 수익 모델을 입증",
      "우체국 안심택배를 통한 배송 프로세스를 직접 구축 및 관리하여 배송 신뢰도와 사용자 경험을 높임",
      "Tailwind CSS와 외부 디자이너 협업을 통해 원페이지 형태의 모바일 친화적 UI/UX 디자인 및 반응형 웹 구축",
      "무통장 입금(팝빌) 및 본인 인증(드림시큐리티) 시스템과 다양한 소셜 로그인(구글, 카카오, 네이버, 애플)을 연동하여 사용자 접근성과 편의성을 향상",
      "웹-앱 브릿지 연동 및 Google Analytics 통합을 통해 네이티브 앱 환경에서의 데이터 통신을 최적화하고, 사용자 유입 경로 분석 기반의 서비스 운영 지원",
    ],
    links: {
      notion:
  ...
  },
  {
    ...
    works: [
      ...
      "Socket.IO를 활용하여 관리자 간 실시간 채팅 기능 구현으로 신속한 내부 커뮤니케이션 지원",
      "사용자 피드백 및 시스템 오류를 빠르게 대응하고 처리하여 시스템 안정성 향상",
      "개발팀과 해당 업체 간 소통을 직접 담당하여 효율적인 이슈 해결과 업무 진행 지원",
    ],
    links: {
      notion:
  ...
  },
  {
    ...
    works: [
      "클라이언트와 기획 단계에서부터 정책 관련 소통을 담당하고 불명확한 프로세스를 협의하여 명확한 개발 기준 마련",
      "장바구니, 결제, 반품/교환을 포함한 주요 서비스 페이지 및 관리자 사이트 구현",
      "카카오, 구글 소셜 로그인과 KG 이니시스 결제 시스템을 성공적으로 연동하여 사용자 편의성 향상",
      "Nest.js와 Prisma를 활용하여 테이블 추가 및 서버 CRUD 작업을 효율화하여 전체 개발 기간 단축",
      "React-Query 및 Zustand를 활용한 데이터 상태 관리 최적화로 API 호출 빈도를 최소화하고 렌더링 성능을 개선",
    ],
    links: {
      notion:
